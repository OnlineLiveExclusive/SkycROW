...
(function(){
  ...
  let running=true, paused=false, score=0, high=parseInt(localStor  Play.getItem('gf_high')||'0',10), combo=1, t=0;
  ...
  function restart(){
    high=Math.max(high, Math.floor(score)); localStor  Play.setItem('gf_high', String(high));
    const lb = JSON.parse(localStor  Play.getItem('gf_leaderboard')||'[]'); lb.push({score:Math.floor(score), at:new Date().toISOString()}); lb.sort((a,b)=>b.score-a.score);
    localStor  Play.setItem('gf_leaderboard', JSON.stringify(lb.slice(0,50)));
